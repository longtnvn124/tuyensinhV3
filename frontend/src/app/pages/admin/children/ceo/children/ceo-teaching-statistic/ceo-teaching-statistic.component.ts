...
import { cloneDeep , filter as _filter } from 'lodash-es';
...
import { getMonthBoundaryDates , Helper } from '@utilities/helper';
...
@Component( {
	selector    : 'app-ceo-teaching-statistic' ,
	imports     : [ LoadingProgressComponent , DatePicker , EmployeePhotoPipe , FindInArrayPipe , NgScrollbar , MatMenu , MatMenuItem , FormsModule , MatMenuTrigger , ClassTimeSlotInfoPipe , NgTemplateOutlet , MatButton , MatTooltip , DatePipe ] ,
	templateUrl : './ceo-teaching-statistic.component.html' ,
	styleUrl    : './ceo-teaching-statistic.component.css'
} )
export default class CeoTeachingStatisticComponent implements OnDestroy {
	...
	private loadData() : void {
		...
		if ( this.branchID() ) {
			joinSources<{
				...
				// 		if ( classSession.teacher_id || classSession.assistant_id ) {
				// 			const index : number = reducer.findIndex( ( t : TeachingStatistic ) : boolean => t.employee.user_id === classSession.teacher_id || t.employee.user_id === classSession.assistant_id );
				// 			if ( -1 !== index ) {
				// 				const dayOfWeek : number = new Date( classSession.time_start ).getDay();
				// 				switch ( dayOfWeek ) {
				// 					case 1:
				// 						reducer[ index ].monday.push( classSession );
				// 						break;
				// 					case 2:
				// 						reducer[ index ].tuesday.push( classSession );
				// 						break;
				// 					case 3:
				// 						reducer[ index ].wednesday.push( classSession );
				// 						break;
				// 					case 4:
				// 						reducer[ index ].thursday.push( classSession );
				// 						break;
				// 					case 5:
				// 						reducer[ index ].friday.push( classSession );
				// 						break;
				// 					case 6:
				// 						reducer[ index ].saturday.push( classSession );
				// 						break;
				// 					default:
				// 						reducer[ index ].sunday.push( classSession );
				// 						break;
				// 				}
				// 				switch ( true ) {
				// 					case ( classSession.parent_id !== 0 ):
				// 						reducer[ index ].totalSupportSessions += 1;
				// 						break;
				// 					case ( classSession.learning_mode === 'one_on_one' ):
				// 						reducer[ index ].totalOneOnOneSessions += 1;
				// 						break;
				// 					case ( reducer[ index ].employee.user_id === classSession.teacher_id ):
				// 						reducer[ index ].totalLectureSessions += 1;
				// 						break;
				// 					default:
				// 						reducer[ index ].totalTeachingAssistantSessions += 1;
				// 						break;
				// 				}
				// 			}
				// 		}
				...
				// } )
				map( ( { teachingStatistics , classSessions } : { teachingStatistics : TeachingStatistic[], classSessions : ClassSessionExtends[] } ) : TeachingStatistic[] => {
					return teachingStatistics.map( ( staff : TeachingStatistic ) : TeachingStatistic => {
						if ( !staff.employee.user_id ) {
							return staff;
						}
						const userID : number                       = staff.employee.user_id;
						const staffSessions : ClassSessionExtends[] = _filter( classSessions , ( s : ClassSessionExtends ) : boolean => userID === s.teacher_id || userID === s.assistant_id );
						staffSessions.forEach( ( _classSession : ClassSessionExtends ) : void => {
							const dayOfWeek : number = new Date( _classSession.time_start ).getDay();
							switch ( dayOfWeek ) {
								case 1:
									staff.monday.push( _classSession );
									break;
								case 2:
									staff.tuesday.push( _classSession );
									break;
								case 3:
									staff.wednesday.push( _classSession );
									break;
								case 4:
									staff.thursday.push( _classSession );
									break;
								case 5:
									staff.friday.push( _classSession );
									break;
								case 6:
									staff.saturday.push( _classSession );
									break;
								default:
									staff.sunday.push( _classSession );
									break;
							}
							switch ( true ) {
								case ( _classSession.parent_id !== 0 ):
									staff.totalSupportSessions += 1;
									break;
								case ( _classSession.learning_mode === 'one_on_one' ):
									staff.totalOneOnOneSessions += 1;
									break;
								case ( userID === _classSession.teacher_id ):
									staff.totalLectureSessions += 1;
									break;
								default:
									staff.totalTeachingAssistantSessions += 1;
									break;
							}
						} );
						return staff;
				...
				next  : ( data : TeachingStatistic[] ) : void => {
					this.teachingStatistics.set( data );
