...
import { LopHocSearchInfo } from '@app/services/lop-hoc.service';
...
export default class SalesDataComponent implements OnInit , OnDestroy , IctuBasePermission {
	...
	private helper : HelperClass                         = new HelperClass();
	...
	readonly importFiltered : Signal<HSImport[]> = computed( () : HSImport[] => {
		return this.isListImportDuplicate()
		       ? this.listImport().filter( ( item : HSImport ) : boolean => item.state == 'invalid' )
		       : this.listImport();
	} );
		...
		SUBMIT_FORM          : () : void => {
			if ( this.formControl.canSubmit ) {
				const info : Partial<SaleData>       = {
					...
					dob          : this.helper.formatSQLTimeStamp(
					...
					child_dob    : this.helper.formatSQLTimeStamp(
						this.formField( 'child_dob' ).value
					) ,
					status       : this.formField( 'status' ).value ,
					donvi_id     : this.donviId
				};
				const request : Observable<SaleData> = this.formControl.isFormAdd
	...
	permissionControl : Signal<IctuPermissionControl> = signal<IctuPermissionControl>( new IctuPermissionControl( this.auth.getUserPermission( 'dao-tao/hoc-sinh' ) ) );
	...
	private requestDeletingData ( ids : number[] ) : void {
		this.notification.confirmDelete( ids.length ).pipe(
			...
			map( () : IctuDeletingAnimationControl<SaleData> => new IctuDeletingAnimationControl( ids , this.service ) ) ,
			switchMap( ( deleteController : IctuDeletingAnimationControl<SaleData> ) : Observable<boolean> => {
	...
	joinTTHS ( saleData : SaleData ) : string {
		return (
			...
			' - ' +
			this.helper.formatSQLDate( new Date( saleData.child_dob ) )
		);
	}
	...
	downloadFileImportSample () : void {
		this.notification.downloadLocalFile( 'files/samples/import-HocSinh.xlsx' , 'dacms-file-mau-import-hoc-sinh' );
	}
	...
	getIDDropDown ( option : IctuDropdownOption<number>[] , label : string ) {
		var tam = option.find( ( item ) => item.label.toLowerCase().includes( label.toLowerCase() ) );
