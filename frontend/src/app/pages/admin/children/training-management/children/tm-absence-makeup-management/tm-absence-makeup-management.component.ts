...
@Component( {
    selector    : 'app-tm-absence-makeup-management' ,
    standalone  : true ,
    imports : [ IctuPaginatorComponent , LoadingProgressComponent , MatButton , MatCheckbox , MatTooltip , ReactiveFormsModule , SharedModule , DatePicker , Tooltip , SafeUrlPipe , DatePipe , FindInArrayPipe , NgClass , Dialog , Str2datePipe , StaffControlComponent , EmployeePhotoPipe , IctuCustomMatMenuComponent , StudentAvatarPipe , Select ] ,
    templateUrl : './tm-absence-makeup-management.component.html' ,
    styleUrl    : './tm-absence-makeup-management.component.css'
} )
export default class TmAbsenceMakeupManagementComponent implements OnDestroy {
    ...
    protected readonly rangeDates : WritableSignal<Date[]> = signal( getWeekBoundaryDates( new Date() ) );
    ...
    readonly relatedClasses : WritableSignal<AbsenceMakeupManagementRelatedClass[]> = signal<AbsenceMakeupManagementRelatedClass[]>( [] );
    ...
    // private readonly advanceFiltersTrigger : Signal<MatMenuTrigger> = viewChild( 'advanceFiltersTrigger' , { read : MatMenuTrigger } );

    // private readonly filterDrawer : Signal<Drawer> = viewChild( 'filterDrawer' , { read : Drawer } );
    ...
    readonly classFilterOptions : Signal<IctuDropdownOptionElement<number>[]> = computed( () : IctuDropdownOptionElement<number>[] => {
        return [ { value : 0 , label : 'Tất cả các lớp học' , disabled : false } , ... ( this.relatedClasses().length ? sortBy( this.relatedClasses().map( ( { id , name } : AbsenceMakeupManagementRelatedClass ) : IctuDropdownOptionElement<number> => ( { value : id , label : name , disabled : false } ) ) , 'label' ) : [] ) ];
    } );
    ...
    constructor () {
        merge<[ number , number , number , Date[] ]>(
            ...
            toObservable( this.filterClassID ).pipe(
                debounceTime( 100 )
            ) ,
            toObservable( this.filterProgress ).pipe(
                debounceTime( 100 )
            ) ,
            toObservable( this.rangeDates ).pipe(
                ...
                debounceTime( 100 ) ,
                distinctUntilChanged( ( previous : Date[] , current : Date[] ) : boolean => previous[ 0 ].getTime() === current[ 0 ].getTime() && previous[ 1 ].getTime() === current[ 1 ].getTime() )
            )
        ).pipe(
            takeUntilDestroyed() ,
            map( () : SearchInfo => ( {
            ...
            // filter( ( { branchID , from , to } : SearchInfo ) : boolean => !! branchID && !! from && !! to ) ,
            debounceTime( 100 ) ,
            distinctUntilChanged( ( p : SearchInfo , c : SearchInfo ) : boolean => [ p.branchID , p.from , p.to , p.class_id , p.progress ].join( '' ) === [ c.branchID , c.from , c.to , c.class_id , c.progress ].join( '' ) )
        ).subscribe( () : void => {
            this.loadData( 1 , true );
        } );

        // toObservable( this.filterDrawer ).pipe(
        //     takeUntilDestroyed() ,
        //     filter( Boolean )
        // ).subscribe( ( _drawer  : Drawer ) : void => {
        //     _drawer.onHide.asObservable().pipe(
    ...
    private loadData ( paged : number = 1 , resetPaginator : boolean = true ) : void {
        ...
        if ( this.branchID() ) {
            ...
            if ( this.filterClassID() ) {
                conditions.push( { conditionName : 'class_id' , condition : IctuQueryCondition.equal , value : this.filterClassID().toString() , orWhere : 'and' } )
            }
            const queryParams : IctuQueryParams = {
                limit      : this.dataTable.paginator.rows() ,
                paged ,
                include    : [ 'UNEXCUSED' , 'EXCUSED' , 'NOT_ATTENDED_YET' ].join( ',' ) ,
                include_by : 'status' ,
                with       : 'class_session'
            };
    ...
    private loadRelatedSources ( info : DtoObject<DiemDanh[]> ) : Observable<DtoObject<CheckIn[]>> {
        ...
        return joinSources<{
            ...
            classes   : this.loadRelatedClass( classIDs ) ,
    ...
    private loadRelatedClass ( filterByIDs : number[] ) : Observable<AbsenceMakeupManagementRelatedClass[]> {
        let _loader : Observable<AbsenceMakeupManagementRelatedClass[]>;
        if ( this.relatedClasses().length ) {
            _loader = of( this.relatedClasses() );
        }
        else {
            const conditions : IctuConditionParam[] = [
                { conditionName : 'donvi_id' , condition : IctuQueryCondition.equal , value : this.donViID.toString() }
            ...
            _loader                                 = this.classesService.query( conditions , _queryParams ).pipe(
                map( ( response : DtoObject<Class[]> ) : AbsenceMakeupManagementRelatedClass[] => {
                    this.relatedClasses.set( response.data );
        ...
        return _loader.pipe(
            map( ( response : AbsenceMakeupManagementRelatedClass[] ) : AbsenceMakeupManagementRelatedClass[] => filterByIDs.length ? response.filter( ( _class : AbsenceMakeupManagementRelatedClass ) : boolean => filterByIDs.includes( _class.id ) ) : [] )
        );
    }
    ...
    // protected btnResetAdvanceFilters () : void {
    //     this.formAdvanceSearch.reset( {
    //         progress : 0 ,
    //         class_id : null
    //     } );
    //     this.filterProgress.set( 0 );
    //     this.filterClassID.set( null );
    //     this.advanceFiltersTrigger()?.closeMenu();
    // }
    //
    // protected btnApplyAdvanceFilter () : void {
    //     this.filterClassID.set( this.formAdvanceSearch.get( 'class_id' ).value );
    //     this.filterProgress.set( this.formAdvanceSearch.get( 'progress' ).value );
    //     this.advanceFiltersTrigger()?.closeMenu();
    // }
    ...
    // protected changeAdvanceSearchForm<T extends keyof FormAdvanceSearchFields> ( field : T , value : FormAdvanceSearchFields[T] ) : void {
    //     this.formAdvanceSearch.get( field ).setValue( value );
    // }
