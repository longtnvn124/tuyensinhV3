...
type PlateColorName = 'emerald' | 'green' | 'lime' | 'red' | 'orange' | 'amber' | 'yellow' | 'teal' | 'cyan' | 'sky' | 'blue' | 'indigo' | 'violet' | 'purple' | 'fuchsia' | 'pink' | 'rose' | 'slate' | 'gray' | 'zinc' | 'neutral' | 'stone';

interface PlateColorItem {
    value : string;
    depth : string;
}
...
export default class ColorComponent {
    readonly primaryColor : ColorInfo[]   = [
        {
            value     : 'bg-primary-50' ,
            title     : 'Blue-1' ,
            hexCode   : '#E9F0FF' ,
    ...
    readonly secondaryColor : ColorInfo[] = [
        {
            value     : 'bg-accent-100' ,
            title     : 'Grey-1' ,
            hexCode   : '#F8F9Fa' ,
    ...
    readonly successColor : ColorInfo[]   = [
        {
    ...
    readonly warningColor : ColorInfo[]   = [
        {
    ...
    colorPlates : WritableSignal<PlateColor[]>;
    ...
    constructor () {
        ...
        const colorDeepNumbers : string[]        = [ '50' , '100' , '200' , '300' , '400' , '500' , '600' , '700' , '800' , '900' , '950' ];
        const colorPlateNames : PlateColorName[] = [ 'emerald' , 'green' , 'lime' , 'red' , 'orange' , 'amber' , 'yellow' , 'teal' , 'cyan' , 'sky' , 'blue' , 'indigo' , 'violet' , 'purple' , 'fuchsia' , 'pink' , 'rose' , 'slate' , 'gray' , 'zinc' , 'neutral' , 'stone' ];
        const plates : PlateColor[]              = colorPlateNames.reduce( ( reducer : PlateColor[] , name : PlateColorName ) : PlateColor[] => {
            const colors : PlateColorItem[] = colorDeepNumbers.reduce( ( _colors : PlateColorItem[] , depth : string ) : PlateColorItem[] => {
                return [ ... _colors , { value : `var(--p-${ name }-${ depth })` , depth } ];
            } , new Array<PlateColorItem>() );
            return [ ... reducer , { name , colors } ];
        } , new Array<PlateColor>() );

        this.colorPlates = signal<PlateColor[]>( plates );
    }

    copyContent ( { hexCode , title } : ColorInfo ) : void {
        void this._copyContent.set( {
            value : hexCode ,
            body  : `${ title } : ${ hexCode }`
        } );
    }

    copyColor ( { value , depth } : PlateColorItem , packName : string ) : void {
        void this._copyContent.set( {
            value ,
            body : `${ packName } ${ depth } : ${ value }`
        } );
    }
