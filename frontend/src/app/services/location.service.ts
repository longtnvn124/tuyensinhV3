...
export class LocationService extends IctuBaseServiceClass<Locations> {
	...
	public queryLocation ( conditions : IctuConditionParam[] , queryParams? : IctuQueryParams , table? : string , context? : HttpContext ) : Observable<DtoObject<Locations[]>> {
		const params : HttpParams = paramsConditionBuilder( conditions , new HttpParams( { fromObject : queryParams || {} } ) );
		return this.http.get<DtoObject<Locations[]>>( ''.concat( ENVIRONMENT.deployment.api , table.replace( /\//g , '' ).trim() , '/' ) , { params , context } );
	}
}
